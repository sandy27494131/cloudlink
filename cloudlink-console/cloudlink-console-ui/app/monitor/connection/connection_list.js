...
/* Controllers */
angular.module('app').controller('connctionListMonitorCtrl', ['$scope', '$translate', '$localStorage', 'HttpUtil', '$modal','$timeout','$interval','$stateParams','$rootScope',
    function($scope,$translate,$localStorage,HttpUtil,$modal,$timeout,$interval,$stateParams,$rootScope ) {
		...
		/*排序参数*/
		$scope.orderName='recv_oct_details.rate';
        ...
        $scope.refresh=function(area) {
        	HttpUtil.ajax({
                url : "/api/monitor/connection/" + area,
                async: false,
                data : null,
                method : "GET",
                success : function(result) {
