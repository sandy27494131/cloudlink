...
/* Controllers */
angular.module('app').controller('queueDetailMonitorCtrl', ['$scope', '$stateParams','$translate', '$localStorage', 'HttpUtil', '$modal','$timeout','$interval','$rootScope',
    function($scope,$stateParams,$translate,$localStorage,HttpUtil,$modal,$timeout,$interval ,$rootScope) {
		...
		$rootScope.currentNav=$stateParams.queue;
         ...
         * @param areaCode
         */
        $scope.refreshInfoByArea=function(){
            HttpUtil.ajax({
                url : "/api/monitor/queue/"+$scope.selectedArea+"/"+$stateParams.queue+$rootScope.getChartRange(),
                data : null,
                method : "GET",
                success : function(result) {
                    ...
                    $scope.$apply(function(){
                        var queue=JSON.parse(result.data.queue);
                        $scope.queue=queue;
                        $scope.bindings=JSON.parse(result.data.bindings);
                        $rootScope.initOrUpdateChart($scope.chartQueueMsg,$scope.seriesQueueMsg,queue,false,true);
                        //初始化消息消费率图
                        $rootScope.initOrUpdateChart($scope.chartMsgRate,$scope.seriesMsgRate,queue.message_stats,true,true);
                    });
        ...
        $scope.initInfoByArea=function(){
            HttpUtil.ajax({
                url : "/api/monitor/queue/"+$stateParams.area+"/"+$stateParams.queue+$rootScope.getChartRange(),
                data : null,
                method : "GET",
                success : function(result) {
                    $scope.$apply(function(){
                        ...
                        $scope.bindings=JSON.parse(result.data.bindings);
                        //初始化队列消息图
                        $rootScope.initOrUpdateChart($scope.chartQueueMsg,$scope.seriesQueueMsg,queue,false,false);
                        //初始化消息消费率图
                        $rootScope.initOrUpdateChart($scope.chartMsgRate,$scope.seriesMsgRate,queue.message_stats,true,false);
                        //删除HighCharts的logo→_→
