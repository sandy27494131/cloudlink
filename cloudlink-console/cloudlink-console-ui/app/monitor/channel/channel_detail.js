...
/* Controllers */
angular.module('app').controller('channelDetailMonitorCtrl', ['$scope', '$stateParams','$translate', '$localStorage', 'HttpUtil', '$modal','$timeout','$interval','$rootScope',
	...
	$rootScope.currentNav=$stateParams.channel;
        ...
        $scope.refreshInfoByArea=function(areaCode){
            HttpUtil.ajax({
                url : "/api/monitor/channel/"+areaCode+"/"+$stateParams.channel+$rootScope.getChartRange(),
                data : null,
                method : "GET",
                success : function(result) {
                    ...
                    $scope.$apply(function(){
                        var channel=JSON.parse(result.data);
                        $scope.channel=channel;
                        //初始化消息消费率图
                        $rootScope.initOrUpdateChart($scope.chartMsgRate,$scope.seriesMsgRate,channel.message_stats,true,true);
                    });
        ...
        $scope.initInfoByArea=function(){
            HttpUtil.ajax({
                url : "/api/monitor/channel/"+$stateParams.area+"/"+$stateParams.channel+$rootScope.getChartRange(),
                data : null,
                method : "GET",
                success : function(result) {
                    $scope.$apply(function(){
                        ...
                        $scope.channel=channel;
                        //初始化消息消费率图
                        $rootScope.initOrUpdateChart($scope.chartMsgRate,$scope.seriesMsgRate,channel.message_stats,true,false);
                        //删除HighCharts的logo→_→
