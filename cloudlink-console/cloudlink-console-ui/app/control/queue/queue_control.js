...
/* Controllers */
angular.module('app').controller('queueControlCtrl', ['$scope', '$translate', '$localStorage', 'HttpUtil', '$modal','$timeout','$interval',
    function($scope,$translate,$localStorage,HttpUtil,$modal,$timeout,$interval ) {
        ...
        $scope.initInfoByAreas=function(){
            HttpUtil.ajax({
                url : "/api/check/queue/"+$scope.selectedAreas.join(','),
                async: false,
                data : null,
                method : "GET",
                success : function(result) {
