...
public class NodeWorkStatusController {
    private static final Logger logger = LoggerFactory.getLogger(NodeWorkStatusController.class);
    @Resource
    ...
    @ResponseBody
    @RequestMapping(value = "/api/alarm/nodeWorkStatus", method = RequestMethod.GET)
    public ResponseBean queryNodeStatus(String code, HttpServletRequest req) {
        AlarmConfigVo alarmConfigVo = null;
        try {
            alarmConfigVo = alarmConfigManager.findByAlarmType(AlarmType.NODE_WORKER_STATUS.name());
        } catch (Exception e) {
            logger.error("query node work status alarm failed", e);
        }
        return ResponseBean.buildSuccess(req, alarmConfigVo);
    }
    @ResponseBody
    @RequestMapping(value = "/api/alarm/nodeWorkStatus", method = RequestMethod.PUT)
    public ResponseBean updateNodeStatus(@RequestBody @Valid AlarmConfigVo alarmConfigVo, BindingResult result, HttpServletRequest req) {
        ...
        {
            logger.error("update node work status alarm failed",e);
