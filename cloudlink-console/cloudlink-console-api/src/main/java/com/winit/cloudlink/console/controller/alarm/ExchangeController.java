...
public class ExchangeController {
    ...
    @ResponseBody
    @RequestMapping(value = "/api/alarm/exchange", method = RequestMethod.GET)
    public ResponseBean queryNodeStatus(String code, HttpServletRequest req) {
        AlarmConfigVo alarmConfigVo = null;
        try {
            alarmConfigVo = alarmConfigManager.findByAlarmType(AlarmType.EXCHANGE_STATUS.name());
        } catch (Exception e) {
            logger.error("query exchange alarm failed", e);
        }
        return ResponseBean.buildSuccess(req, alarmConfigVo);
    }
    @ResponseBody
    @RequestMapping(value = "/api/alarm/exchange", method = RequestMethod.PUT)
    public ResponseBean updateNodeStatus(@RequestBody @Valid AlarmConfigVo alarmConfigVo, BindingResult result, HttpServletRequest req) {
        ...
        {
            logger.error("update exchange alarm failed",e);
