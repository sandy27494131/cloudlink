...
public class NodeStatusController {
    ...
    @ResponseBody
    @RequestMapping(value = "/api/alarm/nodeStatus", method = RequestMethod.GET)
    public ResponseBean queryNodeStatus(String code, HttpServletRequest req) {
        AlarmConfigVo alarmConfigVo = null;
        try {
            alarmConfigVo = alarmConfigManager.findByAlarmType(AlarmType.CLUSTER_NODE_STATUS.name());
        } catch (Exception e) {
            logger.error("query node status alarm failed", e);
        }
        return ResponseBean.buildSuccess(req, alarmConfigVo);
    }
    @ResponseBody
    @RequestMapping(value = "/api/alarm/nodeStatus", method = RequestMethod.PUT)
    public ResponseBean updateNodeStatus(@RequestBody @Valid AlarmConfigVo alarmConfigVo, BindingResult result, HttpServletRequest req) {
        ...
        {
            logger.error("update node status alarm failed",e);
