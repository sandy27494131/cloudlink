...
public class ShovelController {
    private static final Logger logger = LoggerFactory.getLogger(ShovelController.class);
    @Resource
    private AlarmConfigManager alarmConfigManager;
    @Resource
    private TaskScheduler taskScheduler;
    @ResponseBody
    @RequestMapping(value = "/api/alarm/shovel", method = RequestMethod.GET)
    public ResponseBean queryNodeStatus(String code, HttpServletRequest req) {
        AlarmConfigVo alarmConfigVo = null;
        try {
            alarmConfigVo = alarmConfigManager.findByAlarmType(AlarmType.SHOVEL_STATUS.name());
        } catch (Exception e) {
            logger.error("query shovel alarm failed", e);
        }
        return ResponseBean.buildSuccess(req, alarmConfigVo);
    }
    @ResponseBody
    @RequestMapping(value = "/api/alarm/shovel", method = RequestMethod.PUT)
    public ResponseBean updateNodeStatus(@RequestBody @Valid AlarmConfigVo alarmConfigVo, BindingResult result, HttpServletRequest req) {
        ...
        {
            logger.error("update shovel alarm failed",e);
